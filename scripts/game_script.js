...
function myFunction(xml)
{
    ...
    hSubs = xmlDoc.getElementsByTagName("HomeSubDetails");
    aSubs = xmlDoc.getElementsByTagName("AwaySubDetails");
...
//loop through list of teams and if the home team name matches take the capacity of their stadium which is used to generate a pie chart later
        ...
        for (i = 0; i < hTeam.length; i++) 
            { 
            //sets teams that have played or are currenntly playing    
            teamOne =
            ...
            // stes the score and current time of the match   
            score =
                ...
                //checks to ensure data is avaible before placing the team lineups onto the page  
                ...
                if(homeLD[i].childNodes.length>0)
                  {
                      homeLineupTxt += " <div class=awayG><p>";
                      var hLD = homeLD[i].childNodes[0].nodeValue;
                      var splitHLD = hLD.split(";");
                      splitHLD.reverse();
                      splitHLD.splice(0,1);
                      for(x=0;x<splitHLD.length;x++)
                            {                        
                                homeLineupTxt+= splitHLD[x] + " ";
                            }
                      homeLineupTxt+="</p></div>"+"</br>";
...
//                       homeLineupTxt += " <div class=awayG><p>"+homeLD[i].childNodes[0].nodeValue+"</p></div>"   
                ...
                if(homeLM[i].childNodes.length>0)
                  {    
                      homeLineupTxt += " <div class=awayG><p>";
                      var hLM = homeLM[i].childNodes[0].nodeValue;
                      var splitHLM = hLM.split(";");
                      splitHLM.reverse();
                      splitHLM.splice(0,1);
                      for(x=0;x<splitHLM.length;x++)
                            {                        
                                homeLineupTxt+=splitHLM[x] + " ";
                            }
                      homeLineupTxt+="</p></div>"+"</br>";
//                       homeLineupTxt +=" <div class=awayG><p>"+homeLM[i].childNodes[0].nodeValue+"</p></div>"
                ...
                if(homeLF[i].childNodes.length>0)
                  {    
                      homeLineupTxt += " <div class=awayG><p>";
                      var hLF = homeLF[i].childNodes[0].nodeValue;
                      var splitHLF = hLF.split(";");
                      splitHLF.reverse();
                      splitHLF.splice(0,1);
                      for(x=0;x<splitHLF.length;x++)
                            {                        
                                homeLineupTxt+=splitHLF[x] + " ";
                            }
                      homeLineupTxt+="</p></div>"+"</br>";
//                       homeLineupTxt +=" <div class=awayG><p>"+homeLF[i].childNodes[0].nodeValue+"</p></div>"
                ...
                if(awayLF[i].childNodes.length>0)
                  {
                      awayLineupTxt += " <div class=awayG><p>";
                      var aLF = awayLF[i].childNodes[0].nodeValue;
                      var splitALF = aLF.split(";");
                      splitALF.reverse();
                      splitALF.splice(0,1);
                      for(x=0;x<splitALF.length;x++)
                            {                        
                                awayLineupTxt+=splitALF[x] + " ";
                            }
                      awayLineupTxt+="</p></div>"+"</br>";
//                       awayLineupTxt +=" <div class=awayG><p>"+awayLF[i].childNodes[0].nodeValue+"</p></div>"
                  }
                if(awayLM[i].childNodes.length>0)
                  {
                       awayLineupTxt += " <div class=awayG><p>";
                      var aLM = awayLM[i].childNodes[0].nodeValue;
                      var splitALM = aLM.split(";");
                      splitALM.reverse();
                      splitALM.splice(0,1);
                      for(x=0;x<splitALM.length;x++)
                            {                        
                                awayLineupTxt+=splitALM[x] + " ";
                            }
                      awayLineupTxt+="</p></div>"+"</br>";
//                       awayLineupTxt += " <div class=awayG><p>"+awayLM[i].childNodes[0].nodeValue+"</p></div>"   
                ...
                if(awayLD[i].childNodes.length>0)
                  {    
                       awayLineupTxt += " <div class=awayG><p>";
                      var aLD = awayLD[i].childNodes[0].nodeValue;
                      var splitALD = aLD.split(";");
                      splitALD.reverse();
                      splitALD.splice(0,1);
                      for(x=0;x<splitALD.length;x++)
                            {                        
                                awayLineupTxt+=splitALD[x] + " ";
                            }
                      awayLineupTxt+="</p></div>"+"</br>";
//                       awayLineupTxt +=" <div class=awayG><p>"+awayLD[i].childNodes[0].nodeValue+"</p></div>"
                ...
                if(hSubs[i].childNodes.length>0)
                  { 
                      var hSubText = hSubs[i].childNodes[0].nodeValue;
                      var splitHSubText = hSubText.split(";");
                      splitHSubText.reverse();
                      splitHSubText.splice(0,1);
                      for(x=0;x<splitHSubText.length;x++)
                            {                        
                                hSubtxt+=splitHSubText[x]+"<br>";
                            }
                     document.getElementById("subs").innerHTML += hTeam[0].childNodes[0].nodeValue + "<br>" +hSubtxt;

                      
                  }
                
                if(aSubs[i].childNodes.length>0)
                  { 
                      var aSubText = aSubs[i].childNodes[0].nodeValue;
                      var splitASubText = aSubText.split(";");
                      splitASubText.reverse();
                      splitASubText.splice(0,1);
                      for(x=0;x<splitASubText.length;x++)
                            {                        
                                aSubtxt+=splitASubText[x]+"<br>";
                            }
                     document.getElementById("subs").innerHTML += aTeam[0].childNodes[0].nodeValue + "<br>" +aSubtxt;
                ...
                //same function gathering scorers of home goals but for away goals
                ...
                //same function gathering scorers of home goals but for cards
            ...
            //hides details so they cannot be seen but the data can still be taken
            $("#ACdetails").hide();
            $("#HCdetails").hide();
            $("#AGdetails").hide();
            $("#HGdetails").hide();
            ...
            document.getElementById("score").innerHTML = score;
             document.getElementById("title").innerHTML =teamOne +"-"+homeGoals+"-"+awayGoals+"-"+teamTwo;
